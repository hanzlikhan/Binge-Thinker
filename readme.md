...
SupplyShield is built with a modern, scalable stack to ensure rapid development and robust performance.
...
Streamlit (responsive, rapid prototyping)
...
LangChain (prompt chaining), LangGraph (agent orchestration)
...
Mistral-7B-Instruct-v0.1 (Hugging Face, open-source)
...
ChromaDB (vector database for risk history)
...
Mock APIs (shipment, news, weather); extensible to NewsAPI, OpenWeatherMap
...
Mock Slack/SendGrid APIs (console output); extensible to real APIs
...
Streamlit Cloud (free tier), Render, or Railway for production
...
Color Scheme: Blue (#1E3A8A), white (#FFFFFF), red (#DC2626), orange (#F97316).



Responsiveness: Uses Streamlit’s st.columns and st.container for layout flexibility.



Interactivity: Buttons for data refresh, downloads, and user inputs.
...
Set Up Hugging Face API Key: Create a .env file:
